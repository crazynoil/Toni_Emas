...
<?php
  $result = $connect->query('SELECT p.*,s.nomor_sesi FROM pertanyaan p, sesi s WHERE s.id_soal = 1 AND s.id_sesi = p.id_sesi');
?>
...
<script>
  ...
  $.getJSON('_comm/sesi.php', {'id_soal': 1}, function (data) {
  ...
  function sendFile(file) {
            ...
            $.ajax({
                ...
                success: function(url) {
                    var image = $('<img>').attr('src', url).attr('class', 'responsive');
                    $('#isi_pertanyaan').summernote("insertNode", image[0]);
                },
                error: function(data) {
                  alert(error);
                  console.log(data);
